...
test("getIntercomDirPath points at the intercom runtime directory under the agent dir", () => {
  ...
  assert.equal(getBrokerLogPath(join("/tmp/pi-agent", "intercom")), join("/tmp/pi-agent", "intercom", "broker.log"));
});

test("socket probes only classify missing and refused paths as stale", () => {
