...
### Added
- Custom tools now return structured failures with `isError: true` and machine-readable `details.code` plus target IDs. `get_subagent_result` recovers expired terminal records from persisted session entries and defaults to a bounded `result_mode: "summary"`; callers can explicitly request `"full"` output.
- `get_subagent_result({ agent_ids, wait: true })` races several selected agents and returns whichever completes, fails, or stops first. Queued agents remain eligible after starting, only the winning result is consumed, and interrupting the wait leaves every selected agent running with normal completion delivery intact.
- `aside_subagent({ agent_id, message })` asks a running agent a one-off question without steering, interrupting, or mutating its live session. Answers run in a throwaway in-memory session seeded from a deep-cloned snapshot of finalized child context, using the child's effective model, model runtime, system prompt, and worktree cwd with only `read`, `ls`, `find`, and `grep`. Asides time out after 120 seconds, cancel independently with the parent tool or extension shutdown, reject overlap per target, return nested model usage for accounting, and emit privacy-safe `subagents:aside` telemetry without the question or answer. Because in-flight tool results are not finalized yet, an aside can be slightly stale while the child is executing a tool; use `steer_subagent` when the message should intentionally alter the child's work.
