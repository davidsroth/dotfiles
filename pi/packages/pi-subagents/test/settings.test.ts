...
describe("settings persistence", () => {
  ...
  it("loads from global when no project file", () => {
    writeGlobal({ maxConcurrent: 16, schedulingEnabled: false });
    expect(loadSettings(projectDir)).toEqual({ maxConcurrent: 16, schedulingEnabled: false });
  });
  ...
  it("merges global + project with project winning on conflicts", () => {
    writeGlobal({ maxConcurrent: 16, defaultJoinMode: "async" });
    writeProject({ maxConcurrent: 4, schedulingEnabled: false });
    expect(loadSettings(projectDir)).toEqual({
      maxConcurrent: 4, // project wins
      defaultJoinMode: "async", // from global
      schedulingEnabled: false, // from project only
    });
  });
  ...
  it("ignores legacy turn-limit settings", () => {
    writeProject({ defaultMaxTurns: 50, graceTurns: 5 });
    expect(loadSettings(projectDir)).toEqual({});
  });
  ...
  it("composes partial global + partial project correctly", () => {
    writeGlobal({ defaultJoinMode: "group" });
    writeProject({ maxConcurrent: 2 });
    expect(loadSettings(projectDir)).toEqual({ defaultJoinMode: "group", maxConcurrent: 2 });
  });

  describe("sanitizer", () => {
    it("drops maxConcurrent < 1", () => {
      writeProject({ maxConcurrent: 0, defaultJoinMode: "smart" });
      expect(loadSettings(projectDir)).toEqual({ defaultJoinMode: "smart" });
    });
    ...
    it("keeps valid fields while dropping invalid siblings", () => {
      writeProject({
        maxConcurrent: 4, // ok
        defaultMaxTurns: 50, // legacy field, dropped
        graceTurns: 3, // legacy field, dropped
  ...
  describe("applyAndEmitLoaded", () => {
    ...
    it("loads, applies, and emits subagents:settings_loaded with merged settings", () => {
      writeGlobal({ maxConcurrent: 16 });
      writeProject({ defaultJoinMode: "group" });
      const emit = vi.fn();
      ...
      expect(emit).toHaveBeenCalledWith("subagents:settings_loaded", {
        settings: { maxConcurrent: 16, defaultJoinMode: "group" },
      });
      expect(result).toEqual({ maxConcurrent: 16, defaultJoinMode: "group" });
    });
  ...
  describe("saveAndEmitChanged", () => {
    it("persists, emits with persisted=true, and returns info toast on success", () => {
      const emit = vi.fn();
      const snapshot = { maxConcurrent: 5, defaultJoinMode: "smart" as const };
