...
`/agents` promotes the existing `agents` widget into its focused picker state instead of rendering a second panel or overlay. With pi-vim loaded, its literal `Left Arrow` on an empty prompt opens this same picker in either INSERT or NORMAL mode; pi-subagents registers the optional hook only for the current root TUI session and removes it on session switch/shutdown. The same widget slot then expands into the conversation monitor and returns to passive status when closed. The picker and monitor share a rounded window frame, and the monitor renders both finalized and currently streaming thinking blocks alongside normal messages and tool cards. Only running and queued agents appear, and the list updates live. Use `↑`/`↓` or `j`/`k` to choose an agent and Enter to watch it. In the monitor, use `←`/`→` or `h`/`l` to move between active agents, PgUp/PgDn to scroll, End to resume auto-follow, and `s` to compose a steering message inline. Escape returns to the picker; Escape again closes it.
