...
export class ModalEditor extends CustomEditor {
  ...
  private readonly statusColorizers: { insert: (s: string) => string; normal: (s: string) => string } | null;
  private readonly setStatusText: ((text: string | undefined) => void) | null;
  ...
  constructor(
    ...
    super(tui, theme, kb);
    this.setStatusText = setStatusText ?? null;
    this.statusColorizers = statusColorizers ?? null;
  ...
  private syncFooterStatus(): void {
    const rawLabel = this.getModeLabel().trim();
    const colorize = this.statusColorizers
      ? (this.mode === "insert" ? this.statusColorizers.insert : this.statusColorizers.normal)
      : null;
    this.setStatusText?.(colorize ? colorize(rawLabel) : rawLabel);
  }
...
export default function (pi: ExtensionAPI) {
  pi.on("session_start", (_event, ctx) => {
    ...
    const colorizers = t ? {
      insert: (s: string) => t.fg("warning", s),
      normal: (s: string) => t.fg("borderAccent", s),
    } : null;
    ctx.ui.setEditorComponent(
      (tui, theme, kb) =>
        new ModalEditor(
          ...
          theme,
          kb,
          (text) => ctx.ui.setStatus("pi-vim", text),
          colorizers,
        ),
    );
  });
